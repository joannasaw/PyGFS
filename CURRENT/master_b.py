        ...
        def alloc_write(self, dest, num):
            ...
            for i in range(0, num):
                ...
                for i in range(self.__class__.num_replica-1):
                    secondary_ids.append(str(nodes_id)+"_"+str(i+1))
                blocks.append((block_uuid, nodes_id, secondary_ids))

                # append block_id , Chunk_server_id, Chunk_server's replicas_ids, index_of_block
                self.__class__.file_table[dest].append((block_uuid, nodes_id, secondary_ids, i))
